...
const createArticleFormNode = $("#createArticleForm");
...
$(function () {
  ...
  createArticleFormNode.validate({
  ...
  // Handle submit create article form
  createArticleFormNode.submit(function (ev) {
    handleSubmitCreateArticleForm(ev, 1);
  });

  $("#btn-save-draft").on("click", function (ev) {
    handleSubmitCreateArticleForm(ev, 0);
  });

  // Handle reset create article form
  createArticleFormNode.on("reset", handleResetCreateArticleForm);
});

function handleSubmitCreateArticleForm(ev, articleStatus) {
  ev.preventDefault();

  if (!createArticleFormNode.valid()) {
    return;
  }
  ...
  $.ajax({
    ...
    .done(function (resp, textStatus, jqXHR) {
      if (resp.status === "success") {
        createArticleFormNode.trigger("reset");
        Swal.fire({
          title: resp.msg,
...
function handleResetCreateArticleForm() {
  articleContentEditor.setData("");
  articleCategoryNode.val("0").trigger("change");
  articleTagsNode.val("").trigger("change");
  articleThumbnail.removeAllFiles(true);
}
